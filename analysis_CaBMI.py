# general program for all analysis it may plot something, but mostly saves it somewhere 
...
def learning(folder, animal, day, sec_var='', toplot=True):
    # function to check learning rate
    ...
    if toplot:
        ...
        sns.regplot(xx[1:-np.where(xx>blen)[0][0]]/60, hpm[np.where(xx>blen)[0][0]:], label='hits per min')
        ax.set_xlabel('hpm')
...
def activity_hits(folder, animal, day, sec_var=''):
    # Function to obtain the activity of neurons time-lock to end of trial
    folder_path = folder +  'processed/' + animal + '/' + day + '/'
...
#def neuron_activity(folder, animal, day, sec_var='', toplot=True):
...
def frequency_tunning(folder, animal, day, toplot=True):
    #function to check if there is any tunning to different frequencies
    folder_path = folder +  'processed/' + animal + '/' + day + '/'
    ...
    tunning_session = np.ones((dff.shape[0], frequencies.shape[0])) + np.nan   #(neurons, frequencies)
    tunning_session_er = np.ones((dff.shape[0], frequencies.shape[0])) + np.nan   #(neurons, frequencies)
    tunning_trial = np.ones((dff.shape[0], end_trial.shape[0], frequencies.shape[0])) + np.nan  #(neurons, trials, frequencies)
    ...
    if frequency.shape[0] != dff.shape[1]:
        print('Warnng arrays length mismatch')
    ...
    for ind,freq  in enumerate(frequencies):
        ...
        tunning_session[:, ind] = np.nanmean(dff[:, ind_freq], 1)
        tunning_session_er[:, ind] = pd.DataFrame(dff[:, ind_freq]).sem(0).values # std of the mean
    ...
    for tt, trial in enumerate(trials):
        ...
        for ind,freq  in enumerate(frequencies):
            ...
            tunning_trial[:, tt, ind] = np.nanmean(aux_dff[:, ind_freq], 1)
            ind_ts[tt, ind] = ind_freq.shape[0]
    
    f.create_dataset('tunning_session', tunning_session)
    f.create_dataset('tunning_trial', tunning_trial)
    f.create_dataset('ind_ts', ind_ts)
    f.create_dataset('ind_tt', ind_tt)
    
    if toplot:
        for n in np.arange(dff.shape[0]):
            sm_data = ut.sliding_mean(tunning_session[n,:], window=window_sld)
            sm_error = sliding_mean(tunning_session_er[n,:], window=window_sld)
            plt.fill_between(frequency_values, sm_data - sm_error, sm_data + sm_error, color="#3F5D7D")
            plt.plot(frequency_values, sm_data, color="white", lw=1)
            plt.savefig(folder_dest + day + '_' + n + '_smtuning_curve.png', bbox_inches="tight")


# def prediction(folder, animal, day, sec_var='', toplot=True):
    # Can we predict the result of the hit by looking at the activity of neurons?
    # separate ensamble/indirect neurons, IT/PT/REST
    

# def tdmodel(folder, animal, day, sec_var='', toplot=True):
    # Create TD model and compare V(t) and d(T) to activity of neurons
    # IT/PT/REST
    
    
# def general_TE(folder, animal, day, sec_var='', toplot=True):
    # Function to calculate the TE amond neurons 
    # separate ensamble/indirect neurons, hits/miss, IT/PT/REST
